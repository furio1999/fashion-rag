...
# We modified the file to adapt the inpainting pipeline to the MGDV2 model.
...
class FashionRAG_Pipe(DiffusionPipeline):
    r"""
    ...
    def __call__(
            ...
            attention_layers_fine_dict: dict[int, tuple[int]],
            ...
            eta: float = 0.0,
            prompt_embeds: Optional[torch.FloatTensor] = None,
            prompt_embeds_fine: torch.Tensor = None,
            negative_prompt_embeds: Optional[torch.FloatTensor] = None,
            ...
            sketch: Union[torch.FloatTensor, PIL.Image.Image] = None,
            no_pose: bool = False,
    ):
        r"""
        ...
        Args:
            ...
            attention_layers_fine_dict 
                A dictionary with key as the crossattn posititon at which starting conditioning at the specified layers. Every layer can have
                multiple attention modules. dictionary values: the speficied layers in the Unet. 
        ...
        # 3a. Encode input prompt for fine
        if prompt_embeds_fine is not None:
            prompt_embeds_fine = self._encode_prompt(
                None,
                device,
                num_images_per_prompt,
                do_classifier_free_guidance,
                None,
                prompt_embeds=prompt_embeds_fine,
                negative_prompt_embeds=negative_prompt_embeds,
            )
        ...
        if sketch is not None:
            sketch_latents = torchvision.transforms.functional.resize(
        ...
        if sketch is not None:
            sketch_latents = torch.cat([torch.zeros_like(sketch_latents),
                                        sketch_latents]) if do_classifier_free_guidance else sketch_latents

        # 8. Check that sizes of mask, masked image and latents match
        num_channels_mask = mask.shape[1]
        num_channels_masked_image = masked_image_latents.shape[1]
        num_channels_pose_map = pose_map.shape[1] if not no_pose else 0
        num_channels_sketch = sketch.shape[1] if sketch is not None else 0
        if num_channels_latents + num_channels_mask + num_channels_masked_image + num_channels_pose_map + num_channels_sketch != self.unet.config.in_channels:
            raise ValueError(
                ...
                f" `num_channels_pose`: {num_channels_pose_map} + `num_channels_sketch`: {num_channels_sketch}"
                f" = {num_channels_latents + num_channels_masked_image + num_channels_mask + num_channels_pose_map + num_channels_sketch}. Please verify the config of"
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # select attention layers for fine features conditioning
                if i in attention_layers_fine_dict.keys():
                    attention_layers_fine = attention_layers_fine_dict[i]
                    
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents

                # 10a. warped cloth conditioning
                if i >= (num_inference_steps - sketch_conditioning_steps) and sketch is not None:
                ...
                if sketch is not None:
                    latent_model_input = torch.cat(
                        [latent_model_input, mask, masked_image_latents, pose_map.to(mask.dtype),
                         sketch_latents.to(mask.dtype)], dim=1)
                else:
                    latent_model_input = torch.cat(
                ...
                noise_pred = self.unet(
                    ...
                    encoder_hidden_states_fine=prompt_embeds_fine, # TODO Need custom Unet
                    attention_layers_fine=attention_layers_fine,
                ).sample
