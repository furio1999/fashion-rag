# From unet_2d_condition.py of diffusers package
#
...
from diffusers.models.cross_attention import AttnProcessor # version 0.14
# from diffusers.models.attention_processor import AttnProcessor # new
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        encoder_hidden_states_fine: torch.Tensor = None,
        attention_layers_fine: Optional[Tuple[int]] = tuple(),
        ...
        Args:
            ...
            encoder_hidden_states (`torch.FloatTensor`): (batch, sequence_length, feature_dim) encoder hidden states for text
            encoder_hidden_states_fine (`torch.FloatTensor`): (batch, sequence_length, feature_dim) encoder hidden states for texture inversion
            attention_layers_fine (`Tuple[int]`, *optional*): layers in which perform texture inversion attention,
                (0, 1, 2, 3). 0 = mid layer, 1 = coarse layers (3 up and 1 down), 2 = (2 up and 2 down), 3 = finest layers (1 up and 3 down)
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                if idl in attention_layers_fine and encoder_hidden_states_fine is not None:
                    local_encoder_hidden_states = encoder_hidden_states_fine
                else:
